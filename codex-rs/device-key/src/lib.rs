...
impl DeviceKeyStore {
    ...
    pub fn sign(
        ...
        let binding = self.provider.binding(&request.key_id)?;
...
fn random_key_id() -> String {
    format!("dk_{}", URL_SAFE_NO_PAD.encode(random::<[u8; 32]>()))
}
...
fn validate_remote_control_client_connection_payload(
    ...
    validate_remote_control_target(&payload.target_origin, &payload.target_path)?;
...
fn validate_remote_control_target(
    ...
    if !REMOTE_CONTROL_CLIENT_PATHS.contains(&target_path) {
        return Err(DeviceKeyError::InvalidPayload(
            "targetPath must be a remote-control client endpoint",
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    impl DeviceKeyProvider for MemoryProvider {
        fn create(
            ...
            self.bindings
                .lock()
                .map_err(|err| DeviceKeyError::Platform(err.to_string()))?
                .insert(request.key_id.to_string(), request.binding.clone());
    ...
    fn create_generates_distinct_key_ids() {
        ...
        assert!(first.key_id.starts_with("dk_"));
        assert!(second.key_id.starts_with("dk_"));
    }
